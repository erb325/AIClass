...
int main()
{
  ...
  for (i = 0 ; i < MAX_CITIES ; i++) {
    printf("%d %d\n", curSolution.cities[i].x, curSolution.cities[i].y);
  }
  printf("%d %d\n", curSolution.cities[0].x, curSolution.cities[0].y);
  printf("\n");
...
//  set xrange [-5:105]
//  set yrange [-5:105]
//  plot 'c:\cygwin\home\Tim\aing\ch3\out.txt using 1:2 t "Cities" ; replot 'c:\cygwin\home\Tim\aing\ch3\out.txt' using 1:2 t "Tour" with lp
//
